...
| 12 | **PPC driver code form**: the `'PPCC'` codeResID resource contains the raw PEF container (PPC code); SUPERSEDED by the fork-framing correction — the earlier reading ("'OMdv' contains a PEF preceded by a 4-byte length header", Roland SC-8850) misread the resource fork's record framing as payload; the SC-8850 `'OMdv'` 128 is a 68K shim and the authentic `'PPCC'` 1 (OMS Time Manager) is the raw PEF (`Joy!peffpwpc` at byte 0) | verified (fork map; loader disassembly) |
