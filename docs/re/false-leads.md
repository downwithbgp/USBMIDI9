...
| F15 | Every `+98A2` callback leaves the Pascal frame for `+98BE/+98C2` | **RETRACTED / QUALIFIED** | the 2026-08-20 transcript shows a real OMdv veneer whose epilogue copies the result, removes `0x0A` argument bytes, restores the caller PC, and returns cleanly; the older zero-ProcInfo descriptor capture remains a separate USB/PPCC-specific observation |
